...
async function fetchCartItems() {
    ...
    try {
        const response = await fetch('https://drf-2nd-ecommerce-backend.vercel.app/dashboard/customer/cart/', {
            method: 'GET',
            headers: {
...
document.getElementById('checkout-button').addEventListener('click', async () => {
    ...
    try {
        const response = await fetch('https://drf-2nd-ecommerce-backend.vercel.app/products/purchase/', {
            method: 'POST',
            headers: {
                'Authorization': `Token ${token}`,
                'Content-Type': 'application/json',
